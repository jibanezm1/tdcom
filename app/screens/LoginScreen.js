...
export default function LoginScreen (props){
    ...
    const consulta = () =>{       
        ...
        fetch("http://142.93.250.218/api/web/index.php/servidores/listar", requestOptions)
    ...
    const renderRow = () =>{
        ...
        return( servidor.map( (x,i) => { 
            return( <Picker.Item label={x.nombre.toString()} value={x.url.toString()} key={x.id}   />)} ));
    ...
    const _storeData = async (result) => {
      ...
      const test = (value) => {
        ...
        fetch(value.url+"/api/dte/contribuyentes/info/"+value.rut, requestOptions)
          .then(response => response.text())
          .then(result => {
    ...
    <KeyboardAwareScrollView>
        <View style={[loginStyles.container, {padding:20}]}>
            ...
            {
                servidor?renderRow():<Picker.Item label="Ninguno" value="Ninguno" />
            }
           
            </Picker>
            <MyTextInput onChangeText={(value) => {
                setRut(value);
                console.log(rut);
            }} keyboardType='numeric' placeholder="Ingrese el rut del contribuyente" image='account-circle' />

            <MyTextInput onChangeText={(value) => {
                setHash(value);
                console.log(hash);
            }} keyboardType='email-address' placeholder="Ingrese Su Hash" image='lock' />
